...
FLAGS := name=$(name).dat dir_out=$(abspath $(dir_out)) ICON=$(abspath icon.png) APP_DESCRIPTION="ShadowNAND installer/updater." APP_AUTHOR="Shadowhand" --no-print-directory
...
.PHONY: all
all: launcher a9lh 2xrsa

.PHONY: launcher
launcher: $(dir_out)/$(name).dat

.PHONY: a9lh
a9lh: $(dir_out)/ShadowNAND_Installer.bin
...
$(dir_out)/ShadowNAND_Installer.bin: $(dir_build)/main.bin $(dir_out)
